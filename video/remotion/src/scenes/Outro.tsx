...
/* End-Karte: links Projekt-Infos & Namen, rechts der QR-Code. Bleibt ruhig
   zum Scannen stehen. Kein KI-Hinweis. */
export const Outro: React.FC = () => {
  const frame = useCurrentFrame();
  const brand = useReveal(10);
  const title = useReveal(34, 46);
  const facts = useReveal(72);
  const url = usePopIn(108, 40);
  const qr = usePopIn(92, 48);
  const urlPulse = 0.85 + 0.15 * Math.abs(Math.sin((frame - 108) / 50));
